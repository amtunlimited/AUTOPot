/*
 * This file automatically produced by MLDK/mprep from:
 *	src/pot.tm
...
void pot P(( double * _tp1, long _tpl1));
...
static const char* evalstrs[] = {
	"WinTest[coord_]:=Module[{answer=WinTestinner[coord]},      Retur",
	"n[{answer[[1]],        Table[answer[[i]],{i,2,4}],        Table[",
	"answer[[i]],{i,5,7}],        Table[answer[[i]],{i,8,10}]       }",
	"];      ]",
	(const char*)0,
	(const char*)0
};
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"WinTestinner[Coord_]", (char *)"{Coord}", 0);
